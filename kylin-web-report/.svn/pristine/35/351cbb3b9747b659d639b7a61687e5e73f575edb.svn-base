...
var year = oDate.getFullYear();  //获取系统年
...
function searchajaxcp(){
	...
	if (seaday!=-1 && seamonth==-1) {
		...
		$.ajax({
			url:basePath+'buData/searchbuachievement.do',
			type:"POST",
...
/*function downLoad(){
	if (dataValidate()==true) {
		...
		var week=$("#week").val();
		var day=$("#day").val();
		window.location.href = basePath+"branchData/export.do?year="+year+"&month="+month+"&week="+week+"&day="+day;
	}
}
*/
